...
function createVehiclePicker(config) {
  ...
  function render() {
    ...
    tbody.innerHTML = rows.map(v => {
      ...
      const lockIcon = associated
        ? `<button type="button" class="lockBtn ${unlocked ? 'unlocked' : ''}" data-id="${v.id}" title="${unlocked ? 'Click to lock this row' : 'Click to unlock and allow un-associating'}">${unlocked ? '🔓' : '🔒'}</button>`
        : '';
      return `
        <tr class="${rowCls}">
    ...
    tbody.querySelectorAll('.lockBtn').forEach(btn => {
      btn.addEventListener('click', () => {
        const v = state.vehicles.find(x => x.id === btn.dataset.id);
        if (!v) return;
        v.unlocked = !v.unlocked;
        render();
      });
    });
